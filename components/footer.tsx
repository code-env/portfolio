  ...
  return (
    <footer className="text-background px-10 xl:px-0">
      ...
      <ul className="flex-1  border-dashed py-20 lg:border-r border-b lg:border-b-0  dark:border-neutral-700/30 border-neutral-300/90 text-lg flex flex-col gap-1 text-neutral-700 dark:text-neutral-400">
        <li className="uppercase dark:text-neutral-700 text-neutral-700/50 font-semibold text-sm mb-2">Projects</li>
        {
          projects.map((social, index) => {
            //some code 

            return (
              <li key={index}>{social.title}</li>
            )
          })
        }
      </ul>
      <ul className="flex-1  border-dashed py-20 lg:border-r border-b lg:border-b-0 dark:border-neutral-700/30 border-neutral-300/90 text-lg flex flex-col gap-1 text-neutral-700 dark:text-neutral-400">
        <li className="uppercase dark:text-neutral-700 text-neutral-700/50 font-semibold text-sm mb-2">Contact</li>
        {
          socials.map((social, index) => {
            //some code 
            return (
              <li key={index}>  <Link href={social.link} target="_blank" key={index} className="flex items-center gap-2">
